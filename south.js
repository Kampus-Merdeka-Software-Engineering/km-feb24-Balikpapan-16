...
async function fetchData() {
    try {
        ...
        processSouthData(data);
    } catch (error) {
        console.error('Error fetching data:', error);
    }
}

// Fungsi untuk memproses data untuk region South
function processSouthData(data) {
    const southData = data.filter(item => item.Region === 'South');

    renderSegmentChart(southData);
    renderCategoryChart(southData);
    renderDiscountChart(southData);
    renderSubCategoryChart(southData);  // Tambahkan fungsi untuk sub-kategori
}
...
function renderSegmentChart(data) {
    ...
    const ctx = document.getElementById('southSegmentChart').getContext('2d');
    new Chart(ctx, {
        type: 'bar',
        data: {
            labels: segments,
...
function renderCategoryChart(data) {
    ...
    const ctx = document.getElementById('southCategoryChart').getContext('2d');
    new Chart(ctx, {
        type: 'bar',
        data: {
            labels: categories,
...
function renderDiscountChart(data) {
    ...
    const ctx = document.getElementById('southDiscountChart').getContext('2d');
    new Chart(ctx, {
        type: 'pie',
        data: {
...
function renderSubCategoryChart(data) {
    ...
    const ctx = document.getElementById('southSubCategoryChart').getContext('2d');
    new Chart(ctx, {
        type: 'bar',
        data: {
